...
function renderLongProposals() {
  const container = $("longProposalCards");
  if (!container) return;

  if (!Array.isArray(proposals) || !proposals.length) {
    container.innerHTML = '<div class="notice">Aucune proposition longue a afficher pour le moment.</div>';
    return;
  }

  container.innerHTML = proposals.map(p => `
    <article class="longCard">
      <div class="longCardHead">
        <span class="badge">${escapeHTML(p[5] || "Niveau non precise")}</span>
        <strong>${escapeHTML(p[1] || "Proposition")}</strong>
      </div>
      <div class="longBlock"><b>Hypothese</b><p>${escapeHTML(p[2] || "-")}</p></div>
      <div class="longBlock"><b>Proposition 1</b><p>${escapeHTML(p[3] || "-")}</p></div>
      <div class="longBlock"><b>Proposition 2</b><p>${escapeHTML(p[4] || "-")}</p></div>
    </article>`).join("");
}
...
function renderLongPlanning(containerId, rows) {
  const container = $(containerId);
  if (!container) return;

  if (!Array.isArray(rows) || !rows.length) {
    container.innerHTML = '<div class="notice">Aucun creneau a afficher dans cette vue longue.</div>';
    return;
  }
  ...
  container.innerHTML = DAYS.map(day => {
    ...
    const dayContent = gyms.map(gym => {
      ...
      return `
        <div class="longGym">
          <h4>${escapeHTML(gym)}</h4>
          ${list.length ? list.map(s => `
            <div class="longSlot slot ${clubClass(s[6])}">
              <div class="top">${escapeHTML(s[4])}-${escapeHTML(s[5])} - ${escapeHTML(s[6] || "Association non precisee")}</div>
              <div class="meta">${escapeHTML(s[7] || "-")}</div>
              <div class="meta">${escapeHTML(s[8] || "-")} - ${escapeHTML(s[9] || "-")}</div>
              ${s[10] ? `<div class="meta">${escapeHTML(s[10])}</div>` : ""}
            </div>`).join("") : '<div class="emptyLine">Aucun creneau</div>'}
        </div>`;
    }).join("");

    return `
      <section class="longDay">
        <h3>${escapeHTML(day)}</h3>
        <div class="longGymGrid">${dayContent}</div>
      </section>`;
  }).join("");
}

function renderLongDraftPlanning() {
  renderLongPlanning("longDraftPlanning", draftSlots);
}
